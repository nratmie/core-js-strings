...
function removeFirstOccurrences(str, value) {
  const firstIdx = str.indexOf(value);
  const pre = str.substring(0, firstIdx);
  const post = str.substring(firstIdx + value.length);
  return str.includes(value) ? pre + post : str;
}
...
function removeLastOccurrences(str, value) {
  const firstIdx = str.lastIndexOf(value);
  const pre = str.substring(0, firstIdx);
  const post = str.substring(firstIdx + value.length);
  return str.includes(value) ? pre + post : str;
}
...
function reverseString(str) {
  return Array.from(str).reduceRight((newStr, l) => newStr + l, '');
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].match(/[aeiouy]/i)) count += 1;
  }
  return count;
}
...
function isPalindrome(str) {
  const removedSymbols = /[,.!? ]/g;
  const newStr = str.toLowerCase().replaceAll(removedSymbols, '');
  const reversedStr = Array.from(newStr).reduceRight(
    (s, l) => s + l.toLowerCase(),
    ''
  );
...
function findLongestWord(sentence) {
  const longWord = sentence
    .split(' ')
    .sort((cur, next) => next.length - cur.length);
...
function invertCase(str) {
  ...
  const reversedWords = wordsArr.map((word) => {
    return word
      .split('')
      .map((l) => {
